...
class WTMFVectorizer(object):
    def __init__(self, input='file', k=100, w_m=0.01,
        ...
        if tokenizer is None:
            self.tokenize = self.default_tokenizer_
        else:
    ...
    def default_tokenizer_(self, string):
        return string.split(' ')


class WTMFTransformer(object):
    def __init__(self, k=100, w_m=0.01,
                 lam=0.1, max_iter=20, verbose=False):
        self.k = k
        self.w_m = w_m
        self.lam = lam
        self.max_iter = max_iter
    ...
    def fit(self, X):
        self.P_ = als_grad_descent(
            X, self.k, self.w_m, self.lam, self.max_iter, 
            verbose=self.verbose)
        return self        

    def transform(self, X):
        return project_Q(X, self.P_, self.k, self.w_m, self.lam)


@cython.boundscheck(False)
cdef project_Q(object X, np.ndarray[DTYPE_t, ndim=2] P,
...
@cython.boundscheck(False)
cdef als_grad_descent(object X, int k, double w_m, 
